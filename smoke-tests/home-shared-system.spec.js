...
test("Employee Home uses one token-only visual source", async () => {
  ...
  expect(source).toContain('jgc-design-system.css?v=10');
  expect(source).toContain('home-design-system.css?v=8');
  ...
  expect(serviceWorker).toContain('"./home-design-system.css?v=8"');
});
...
for (const viewport of [
  ...
  test(`populated Employee Home stays readable and contained on ${viewport.name}`, async ({ browser }) => {
    ...
    // Let the page's own start-up loads finish so they cannot replace the sample data.
