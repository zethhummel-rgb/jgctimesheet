...
for (const width of [768, 1366, 1600]) test(`desktop job rows preserve readable text and compact actions at ${width}px`, async ({ page }, testInfo) => {
  ...
  for (const theme of ['light', 'dark']) {
    ...
    for (const row of await rows.all()) {
      ...
      const menu = row.locator('.job-status-menu');
      await expect(menu.locator('summary')).toHaveText('Change Status');
      await expect(menu).not.toHaveAttribute('open', '');
      await expect(menu.getByRole('button', {includeHidden:true})).toHaveCount(3);
  ...
  await expect(rows.locator('.job-status-action')).toHaveText(['Make active', 'Cancel Project', 'Close — Discuss Invoice']);
...
test('compact job rows grow for long names without clipping content', async ({ page }, testInfo) => {
  ...
  const details = await row.locator('td > strong, td > small, .job-status-action').evaluateAll(items => items.filter(el => el.checkVisibility()).map(el => ({top: el.getBoundingClientRect().top, bottom: el.getBoundingClientRect().bottom, rowTop: el.closest('tr').getBoundingClientRect().top, rowBottom: el.closest('tr').getBoundingClientRect().bottom})));
