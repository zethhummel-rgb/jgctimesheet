...
const colors = { white: "FFFFFFFF", green: "FF92D050", yellow: "FFFFFF00", red: "FFFF0000" };
const excelValue = (value: AccountingCell) => value && typeof value === "object" ? new Date(`${value.date.slice(0, 10)}T00:00:00Z`) : value;
const states = { white: "Active", green: "Closed - ready to invoice", yellow: "Previously handed to accounting", red: "Cancelled" };
...
export async function buildJobAccountingWorkbook(preview: AccountingExportPreview, plan: AccountingExportPlan): Promise<Uint8Array> {
  ...
  workbook.title = `Accounting Job List v${preview.version}`;
  ...
  for (const [year, rows] of [...groups].sort(([a], [b]) => b.localeCompare(a))) {
    const sheet = workbook.addWorksheet(year, { views: [{ state: "frozen", ySplit: 2 }], properties: { defaultRowHeight: 22 }, pageSetup: { orientation: "landscape", paperSize: 5, fitToPage: true, fitToWidth: 1, fitToHeight: 0 } });
    ...
    for (let index = 0; index < rows.length; index++) {
      ...
      source.cells.forEach((value, col) => {
        ...
        cell.alignment = { vertical: "middle", wrapText: true, horizontal: col === 0 ? "left" : [9, 11].includes(col) ? "right" : "center" };
      ...
      const lines = source.cells.map((value, col) => {
        if (col > 0 && col < 5) return 1;
        const text = value && typeof value === "object" ? (col === 13 ? new Date(`${value.date.slice(0, 10)}T00:00:00Z`).toLocaleDateString("en-CA", { timeZone: "UTC", weekday: "long", month: "long", day: "numeric", year: "numeric" }) : value.date) : String(value ?? "");
        return Math.ceil(text.length / (col === 0 ? 54 : widths[col] * 0.9));
      });
      sheet.getRow(number).height = Math.max(22, ...lines.map((count) => count * 16));
    }
    const footer = rows.length + 5;
    [
      `JGC Accounting Job List - Version ${preview.version}. Starting reference: ${preview.sourceName}.`,
      "Green: closed, ready to invoice. Red: cancelled. White: new or changed active project.",
    ...
    ].forEach((line, index) => { sheet.mergeCells(footer + index, 1, footer + index, 16); const cell = sheet.getCell(footer + index, 1); cell.value = line; cell.font = { name: "Arial", size: 10, color: { argb: "FF000000" } }; cell.alignment = { wrapText: true, vertical: "middle" }; sheet.getRow(footer + index).height = 28; });
    sheet.pageSetup.printTitlesRow = "1:2";
    sheet.pageSetup.printArea = `A1:P${footer + 4}`;
    sheet.headerFooter.oddFooter = `&CVersion ${preview.version} | Page &P of &N`;
  }
  const details = workbook.addWorksheet("Download details", { views: [{ state: "frozen", ySplit: 5 }] });
  [12, 38, 39, 32, 32, 48, 18].forEach((width, index) => { details.getColumn(index + 1).width = width; });
  details.mergeCells("A1:G1"); details.getCell("A1").value = `Accounting job-list download - Version ${preview.version}`;
  details.mergeCells("A2:G2"); details.getCell("A2").value = `Reference: ${preview.sourceName}. SHA-256: ${preview.sourceSha256}`;
  details.mergeCells("A3:G3"); details.getCell("A3").value = `${plan.summary.changed} new/changed or starting rows; ${plan.summary.yellow} yellow rows retained. Saved versions never change.`;
  ...
  details.eachRow((row: any, number: number) => { row.height = number <= 5 ? 30 : 32; row.eachCell((cell: any, column: number) => {
    ...
    if (number > 5 && !(cell.value instanceof Date)) row.height = Math.max(row.height, Math.ceil(String(cell.value ?? "").length / (details.getColumn(column).width * 0.9)) * 16);
  }); });
