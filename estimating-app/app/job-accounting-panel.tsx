...
interface SavedVersion { id: string; version: number; file_name: string; file_sha256: string; exported_by_name: string; exported_at: string; summary: AccountingExportPlan["summary"]; rows?: AccountingExportRow[]; file_base64?: string }
...
export function JobAccountingPanel({ workspaceSaved }: { workspaceSaved: boolean }) {
  ...
  async function save() {
    ...
    setCount((current) => Math.max(current, saved.record.version));
  ...
  return <section className="panel job-accounting-panel" aria-label="Accounting job-list downloads">
    <div className="panel-heading"><div><span className="eyebrow">ACCOUNTING HAND-OFF</span><h3>Accounting job-list download</h3><p>Changes and retained yellow rows in the master-list layout. Your existing Excel upload stays in use.</p></div>
    ...
    <div className="job-accounting-body">
      ...
      {(preview || selected) && <div className="job-accounting-preview">
        ...
        {preview && <p>{preview.version === 1 ? `Starting coloured rows from ${preview.sourceName}, plus new changes.` : "Creating a version records the accounting hand-off. Re-download an old version from history without advancing colours."} All years and managers are included regardless of the job-list filters.</p>}
        ...
        {Boolean(summary?.missingFromPortal) && <p>{summary!.missingFromPortal} starting-master rows are not in the current portal. They are retained in this accounting file only.</p>}
        {!rows.length ? <p>No changed jobs or retained yellow rows are available yet.</p> : <>
