...
import {
  ...
  type QuoteStatus,
  type ShopDrawing,
  type ShopDrawingResponsibility,
...
function purchaseOrderSubtotal(purchaseOrder: PurchaseOrder) {
  return Math.round(purchaseOrder.lines.reduce((sum, line) => sum + (Number(line.amount) || 0), 0) * 100) / 100;
}

function frozenPurchaseOrderSnapshot(purchaseOrder: PurchaseOrder) {
  return JSON.stringify({
    ...purchaseOrder,
...
function savedPurchaseOrderSnapshot(snapshot: string): PurchaseOrder | null {
  try {
    const parsed = JSON.parse(snapshot) as PurchaseOrder;
...
export default function EstimateDesk({ currentEstimator = { id: "", name: "Zeth", isAdmin: true } }: { currentEstimator?: CurrentEstimator }) {
  ...
  const savePurchaseOrder = (purchaseOrder: PurchaseOrder) => {
    ...
    setState((current) => {
      ...
      return addActivity(
        next,
        purchaseOrderJob.quoteId,
        existingPurchaseOrder ? "Purchase order revision saved" : "Final purchase order created",
        `${purchaseOrder.number} Rev ${purchaseOrder.revision} for ${purchaseOrder.vendorName}${existingPurchaseOrder ? " remains editable until its PDF is downloaded." : " was saved as Final."}`,
      );
    });
    setPurchaseOrderEditor(null);
  };

  const openPurchaseOrder = (jobId: string, purchaseOrderId: string) => {
  ...
  const downloadAndFinalizePurchaseOrder = async (jobId: string, purchaseOrderId: string) => {
    const current = latestState.current;
    const job = current.jobs.find((item) => item.id === jobId);
    const purchaseOrder = job?.purchaseOrders?.find((item) => item.id === purchaseOrderId);
    if (!job || !purchaseOrder) return;
    const finalizedAt = new Date().toISOString();
    const printable = purchaseOrder.status === "Draft"
      ? { ...purchaseOrder, status: "Issued" as const, finalizedAt, updatedAt: finalizedAt }
      : purchaseOrder;
    const succeeded = await downloadPurchaseOrder(current, job, printable);
    if (!succeeded || purchaseOrder.status !== "Draft") return;
    setState((saved) => addActivity({
      ...saved,
      jobs: saved.jobs.map((item) => item.id !== jobId ? item : {
        ...item,
        purchaseOrders: (item.purchaseOrders ?? []).map((po) => po.id === purchaseOrderId ? printable : po),
      }),
    }, job.quoteId, "Purchase order revision finalized", `${printable.number} Rev ${printable.revision} was downloaded and locked as Final.`));
  };
...
function matchesWorkSearch(query: string, ...values: Array<string | null | undefined>) {
  const haystack = values.filter(Boolean).join(" ").toLocaleLowerCase();
  const compact = haystack.replace(/[^\p{L}\p{N}]/gu, "");
  return query.trim().toLocaleLowerCase().split(/\s+/).filter(Boolean).every((term) => {
    const identifier = term.replace(/[^\p{L}\p{N}]/gu, "");
    return haystack.includes(term) || Boolean(identifier && compact.includes(identifier));
  });
}
        ...
        {searchTerms.length > 0 && (
          <div className="overview-search-results" aria-live="polite">
            <div className="overview-search-summary">
              ...
              <span>Searching {currentEstimator.isAdmin ? "all quotes and CCNs" : "your quotes"} and all jobs</span>
            </div>
  ...
  return (
    <div className="quote-workspace">
      <div className="quote-topline">
        ...
        <div className="quote-identity">
          <div>
            ...
            <p>{changeNotice && linkedJob ? `Job ${linkedJob.jobNumber} · ` : ""}{clientName(state, quote.clientId)}{quote.site ? ` · ${quote.site}` : ""}</p>{quote.address?.trim() && <p className="quote-project-address">{quote.address}</p>}
          </div>
      ...
      <div className="quote-tab-content">
        ...
        {tab === "purchase-orders" && linkedJob && <QuotePurchaseOrders state={state} quote={quote} job={linkedJob} onEditPurchaseOrder={onEditPurchaseOrder} onDownloadPurchaseOrder={onDownloadPurchaseOrder} />}
  ...
  return (
    ...
    <div className="content-grid details-grid-layout">
      ...
      <aside className="panel pricing-controls">
        ...
        <div className="control-row range-control"><span><strong>Project markup</strong><small>Applied unless a line overrides it</small></span><div><input aria-label="Project markup slider" type="range" min="0" max="100" step="1" value={Math.round(quote.defaultMarkup * 100)} disabled={locked} style={{ "--range-fill": `${Math.round(quote.defaultMarkup * 100)}%` } as React.CSSProperties} onChange={(event) => updateField("defaultMarkup", Number(event.target.value) / 100)} /><div className="range-percent-entry"><ClearableNumberInput aria-label="Project markup percentage" inputMode="numeric" min="0" max="100" step="1" value={Math.round(quote.defaultMarkup * 100)} disabled={locked} normalizeValue={(value) => Math.min(100, Math.max(0, Math.round(value)))} formatEditedValue={String} onValueChange={(value) => updateField("defaultMarkup", (value ?? 0) / 100)} /><span>%</span></div></div></div>
        <div className="control-row range-control"><span><strong>Target margin</strong><small>Creates a warning below target</small></span><div><input aria-label="Target margin slider" type="range" min="0" max="60" step="1" value={Math.round(quote.targetMargin * 100)} disabled={locked} style={{ "--range-fill": `${Math.min(100, Math.round(quote.targetMargin * 100) / 0.6)}%` } as React.CSSProperties} onChange={(event) => updateField("targetMargin", Number(event.target.value) / 100)} /><div className="range-percent-entry"><ClearableNumberInput aria-label="Target margin percentage" inputMode="numeric" min="0" max="60" step="1" value={Math.round(quote.targetMargin * 100)} disabled={locked} normalizeValue={(value) => Math.min(60, Math.max(0, Math.round(value)))} formatEditedValue={String} onValueChange={(value) => updateField("targetMargin", (value ?? 0) / 100)} /><span>%</span></div></div></div>
      ...
      <section className="panel form-panel full-span">
        ...
        <div className="proposal-breakdown-controls">
          ...
          {quote.proposalShowCostBreakdown && <div className="proposal-breakdown-options">
            ...
            <div className="proposal-breakdown-option-heading proposal-breakdown-line-heading"><div><strong>Select individual estimate lines to show</strong><small>Each selected line shows its selling price. Choose whether to include the manually entered quote description for Vendor/Sub lines.</small></div><span>{breakdownLineOptions.length} line{breakdownLineOptions.length === 1 ? "" : "s"}</span></div>
            {breakdownLineOptions.length ? <div className="proposal-breakdown-line-list">
              {breakdownLineOptions.map((option) => <div key={option.id} className="proposal-breakdown-line-option" role="group" aria-label={`${option.label} breakdown choices`}><label className="check-field proposal-breakdown-line-choice"><input type="checkbox" checked={breakdownLineIds.includes(option.id)} disabled={locked} onChange={(event) => toggleBreakdownLine(option.id, event.target.checked)} /><span><strong>{option.label}</strong><small>{money(option.amount)} selling price · {option.costType}</small></span></label>{option.costType === "Sub / Vendor" && <label className="check-field proposal-description-choice"><input type="checkbox" aria-label="Include Quote Description" checked={quote.proposalBreakdownDescriptionLineIds?.includes(option.id) ?? false} disabled={locked || !breakdownLineIds.includes(option.id)} onChange={(event) => { const selected = new Set(quote.proposalBreakdownDescriptionLineIds ?? []); if (event.target.checked) selected.add(option.id); else selected.delete(option.id); updateField("proposalBreakdownDescriptionLineIds", [...selected]); }} /><span><strong>Include Quote Description</strong><small>{option.quoteDescription || "Quote description is blank. Enter it on the Estimate tab."}</small></span></label>}</div>)}
      ...
      <section className="panel estimate-panel">
        ...
        <div className="estimate-table-wrap">
          <table className="estimate-table">
            <thead>
              <tr><th>#</th><th>Vendor</th><th>Quote description</th><th>Cost type</th><th>Division</th><th>Qty</th><th>Unit</th><th className="direct-unit-cost-heading">Direct unit cost</th><th className="direct-cost-heading">Direct cost</th><th>Class</th><th>Include</th><th><span className="sr-only">Details</span></th></tr>
            </thead>
            <tbody>
                ...
                return (
                  <Fragment key={line.id}>
                    <tr id={`estimate-line-${line.id}`} className={`${expandedLineId === line.id ? "expanded" : ""} ${!line.included ? "not-included" : ""}`}>
                      ...
                      <td className={line.costType === "Sub / Vendor" ? "vendor-quote-cell" : undefined} data-label={line.costType === "Sub / Vendor" ? "Vendor" : "Description"}>{line.costType === "Sub / Vendor" ? <div className="subcontractor-main-cell"><SearchablePicker value={line.vendorName || (line.vendorId ? state.vendors.find((vendor) => vendor.id === line.vendorId)?.name ?? "" : "")} options={activeSubcontractors(state.vendors).map((vendor) => ({ id: vendor.id, label: vendor.name, detail: vendor.trade }))} disabled={locked} placeholder="Search or type subcontractor" ariaLabel={`Vendor for line ${index + 1}`} allowCustom onChange={(value) => updateLineVendor(line, value)} onSelect={(option) => updateLineVendor(line, option.label)} /><label className="sub-quote-inline-field"><span>Quote # <em>Optional</em></span><input aria-label={`Subcontractor quote # for line ${index + 1}`} value={line.vendorReference} disabled={locked} onChange={(event) => updateLine(line.id, { vendorReference: event.target.value })} placeholder={(line.vendorPricingMode ?? "Quoted") === "Budget" ? "Budget allowance" : "Enter quote #"} /></label></div> : <input className="cell-input description-input" autoComplete="off" value={line.description} disabled={locked} onChange={(event) => { const description = event.target.value; const division = !line.divisionManual ? detectConstructionDivision(description) : null; updateLine(line.id, { description, ...(division ? { division } : {}) }); }} placeholder="Describe the work" />}</td>
                      <td className={`quote-description-cell ${line.costType === "Sub / Vendor" ? "" : "is-empty"}`} data-label="Quote description">{line.costType === "Sub / Vendor" ? <input className="cell-input description-input" aria-label={`Subcontractor quote description for line ${index + 1}`} autoComplete="off" value={line.description} disabled={locked} onChange={(event) => { const description = event.target.value; const division = !line.divisionManual ? detectConstructionDivision(description) : null; updateLine(line.id, { description, ...(division ? { division } : {}) }); }} placeholder="Describe what this quote covers" /> : <span className="not-applicable">—</span>}</td>
...
function PurchaseOrderHistory({ state, job, purchaseOrder }: { state: AppState; job: Job; purchaseOrder: PurchaseOrder }) {
  if (!purchaseOrder.revisions.length) return null;
  return (
    <details className="po-revision-history">
      <summary>History ({purchaseOrder.revisions.length})</summary>
      <div>
        {[...purchaseOrder.revisions].reverse().map((revision) => {
          const snapshot = savedPurchaseOrderSnapshot(revision.snapshot);
          return (
            <article key={revision.id}>
              <span><strong>Revision {revision.revision}</strong><small>Final · {shortDate(revision.finalizedAt)}</small></span>
              <span><strong>{money(revision.subtotal)}</strong><small>Pre-tax</small></span>
              <button type="button" className="button secondary compact" disabled={!snapshot} onClick={() => { if (snapshot) void downloadPurchaseOrder(state, job, snapshot); }}>Download</button>
            </article>
          );
        })}
      </div>
...
function QuotePurchaseOrders({ state, quote, job, onEditPurchaseOrder, onDownloadPurchaseOrder }: {
  state: AppState;
  quote: Quote;
  job: Job;
  onEditPurchaseOrder: (jobId: string, purchaseOrderId: string) => void;
  ...
  return (
    <div className="page-stack quote-po-layout">
      ...
      <section className="quote-po-kpis" aria-label="Purchase order summary">
        ...
        <div><span>Final</span><strong>{issuedCount}</strong><small>Locked commitments</small></div>
        <div><span>Revisions</span><strong>{draftCount}</strong><small>Currently in progress</small></div>
      ...
      <section className="panel subcontract-po-panel quote-po-list-panel">
        <div className="panel-heading">
          <div><span className="eyebrow">PURCHASE ORDER REGISTER</span><h2>Attached purchase orders</h2><p>Final POs stay locked. Start a revision to make changes without replacing the previous version.</p></div>
            ...
            <tbody>{purchaseOrders.map((purchaseOrder) => {
              ...
              return (
                <tr key={purchaseOrder.id}>
                  <td data-label="PO number"><strong>{purchaseOrder.number || "Not assigned"}</strong><small>Revision {purchaseOrder.revision}</small></td>
...
function QuoteProposal({ state, quote }: { state: AppState; quote: Quote }) {
  ...
  const company = {
    ...
    signatory: proposalSignatoryName(state, quote),
  };
      ...
      <article className={`proposal-paper ${style === "jgc-classic" ? "classic-proposal hybrid-classic-proposal" : "modern-proposal"}`}>
        {style === "jgc-classic" ? (
          <>
            ...
            <section className={`hybrid-meta ${projectAddress ? "has-address" : ""}`}>
              <div className="document-client"><span>Prepared for</span><strong>{client?.name || "Client not selected"}</strong>{quote.site?.trim() && <p className="document-site">{quote.site.trim()}</p>}{projectAddress && <p className="document-address">{projectAddress}</p>}{(quote.proposalAttention || client?.contact) && <p>Attention: {quote.proposalAttention || client?.contact}</p>}</div>
              <div className="document-project"><span>Project</span><strong>{quote.project || "Project not named"}</strong>{changeNotice && <p>Change: {quote.changeTitle || "Change not named"}</p>}{quote.reference && <p>Reference: {quote.reference}</p>}</div>
              <div className="document-date"><span>{changeNotice ? approvedChange ? "Approved date" : "CCN date" : "Quote date"}</span><strong>{shortDate(approvedChange ? quote.changeOrder?.approvedDate || quote.quoteDate : quote.quoteDate)}</strong>{!approvedChange && <p>Valid until {shortDate(quote.validUntil)}</p>}{changeNotice && quote.changeRequestedBy && <p>Requested by {quote.changeRequestedBy}</p>}</div>
            </section>
        ...
        {style === "jgc-classic" && (
          <>
            ...
            {sharedOptional}
            {quote.proposalShowCostBreakdown && <section className="proposal-cost-breakdown"><h2>Cost Breakdown</h2>{costBreakdownRows.length ? costBreakdownRows.map((row) => <div key={row.key}><span>{row.label}{row.quoteDescription && <small className="proposal-quote-description">{row.quoteDescription}</small>}</span><strong>{money(row.amount)}</strong></div>) : <p>No cost breakdown lines selected.</p>}<footer><span>{changeNotice ? "Change total" : "Proposal total"}</span><strong>{money(customerDocumentSubtotal)}</strong></footer></section>}
                      ...
                      <td colSpan={9}>
                        <div className="price-edit-panel">
                          ...
                          <div className="form-grid price-edit-grid">
                            ...
                            <label className="field"><span>Default markup</span><div className="input-suffix"><ClearableNumberInput min="0" step="0.5" value={item.markup * 100} onValueChange={(value) => updateItem(item.id, "markup", (value ?? 0) / 100)} /><span>%</span></div></label>
                            ...
                            <label className="field"><span>Pricing year</span><ClearableNumberInput min="2000" max="2100" value={item.pricingYear} emptyValue={2000} normalizeValue={(value) => Math.min(2100, Math.max(2000, Math.round(value)))} onValueChange={(value) => updateItem(item.id, "pricingYear", value ?? 2000)} /></label>
...
const shopDrawingResponsibilities: ShopDrawingResponsibility[] = ["Vendor", "JGC", "Consultant / Client", "Complete"];

function isApprovedShopDrawing(status: ShopDrawingStatus) {
  return status === "Approved" || status === "Approved as noted";
}

function isOpenShopDrawing(status: ShopDrawingStatus) {
  return !isApprovedShopDrawing(status) && status !== "Closed";
}
...
function shopDrawingDraftFrom(drawing: ShopDrawing): ShopDrawingDraft {
  return {
    number: drawing.number,
    title: drawing.title,
    division: drawing.division,
    vendorName: drawing.vendorName,
    consultant: drawing.consultant,
    status: drawing.status,
    responsibility: drawing.responsibility,
    requestedDate: drawing.requestedDate,
    receivedDate: drawing.receivedDate,
    submittedDate: drawing.submittedDate,
    dueDate: drawing.dueDate,
    returnedDate: drawing.returnedDate,
...
function nextShopDrawingNumber(drawings: ShopDrawing[]) {
  const next = drawings.reduce((largest, drawing) => {
    const match = drawing.number.trim().match(/^SD-(\d+)$/i);
...
function shopDrawingDateState(drawing: ShopDrawing) {
  ...
  const due = new Date(`${drawing.dueDate.slice(0, 10)}T12:00:00`);
...
function JobsPage({ state, setState, currentEstimator, directoryActionTarget, workspaceSaved, onNewJob, job, tab, setTab, onOpen, onBack, onAddCost, onCreateChangeNotice, onOpenQuote, onCreatePurchaseOrder, onEditPurchaseOrder, onDownloadPurchaseOrder, portalLabourActuals, jobCostingStatus, jobCostingMessage, onRefreshJobCosting }: {
  ...
  const [statusSavingJobId, setStatusSavingJobId] = useState("");
  const statusSaving = Boolean(statusSavingJobId);
  ...
  const [shopDrawingError, setShopDrawingError] = useState(false);
    ...
    try {
      ...
      setState((current) => ({
        ...
        jobs: current.jobs.map((item) => item.id === job.id ? {
          ...item,
          documentLink: savedLink,
          documentLinkLabel: savedLabel,
          shopDrawings: (item.shopDrawings ?? []).map((drawing) => ({ ...drawing, sharedWithEmployees: false })),
        } : item),
      }));
  ...
  const startNewShopDrawing = () => {
    if (!job) return;
    const draft: ShopDrawingDraft = {
      number: nextShopDrawingNumber(job.shopDrawings ?? []),
      title: "",
      division: "",
      vendorName: "",
      consultant: "",
  ...
  const updateShopDrawingDraft = <K extends keyof ShopDrawingDraft>(field: K, value: ShopDrawingDraft[K]) => {
    setShopDrawingEditor((current) => current ? { ...current, draft: { ...current.draft, [field]: value } } : current);
  };

  const clearEmployeeJobListLink = async () => {
    if (!job?.portalJobId) throw new Error("This drawing cannot be removed from the employee job list until the job is linked to the Portal.");
    const response = await fetch("/api/job-documents", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ portalJobId: job.portalJobId, documentLink: "", documentLinkLabel: "" }),
  ...
  const saveShopDrawing = async () => {
    if (!job || !shopDrawingEditor || shopDrawingAction) return;
    const draft = shopDrawingEditor.draft;
    const title = draft.title.trim();
    const number = draft.number.trim();
    const secureLink = secureExternalHref(draft.oneDriveUrl);
    if (!number || !title) {
    ...
    const duplicateNumber = (job.shopDrawings ?? []).some((drawing) => drawing.id !== shopDrawingEditor.drawingId && drawing.number.trim().toLocaleLowerCase("en-CA") === number.toLocaleLowerCase("en-CA"));
    if (duplicateNumber) {
      setShopDrawingError(true);
      setShopDrawingMessage(`${number} is already used on this job.`);
      return;
    }
    ...
    setShopDrawingMessage("");
    try {
      if (mustUnshare) await clearEmployeeJobListLink();
      const now = new Date().toISOString();
      const vendor = state.vendors.find((item) => item.name.trim().toLocaleLowerCase("en-CA") === draft.vendorName.trim().toLocaleLowerCase("en-CA"));
      const common = {
        number,
        title,
      ...
      setState((current) => ({
        ...
        jobs: current.jobs.map((item) => item.id !== job.id ? item : {
          ...item,
          shopDrawings: shopDrawingEditor.mode === "create"
            ? [...(item.shopDrawings ?? []), savedDrawing]
            : (item.shopDrawings ?? []).map((drawing) => drawing.id === savedDrawing.id ? savedDrawing : drawing),
          ...(mustUnshare ? { documentLink: "", documentLinkLabel: "" } : {}),
        }),
      }));
      setShopDrawingEditor(null);
      setShopDrawingMessage(shopDrawingEditor.mode === "revision"
        ? `${savedDrawing.number} Revision ${savedDrawing.revision} saved. The previous revision remains in history.`
        : shopDrawingEditor.mode === "create"
          ? `${savedDrawing.number} added to the register.`
          : `${savedDrawing.number} updated without creating a revision.`);
    } catch (error) {
      setShopDrawingError(true);
      setShopDrawingMessage(error instanceof Error ? error.message : "The shop drawing could not be saved.");
    } finally {
      setShopDrawingAction(null);
    }
  };
  ...
  const shareShopDrawing = async (drawing: ShopDrawing) => {
    ...
    setShopDrawingAction("sharing");
    setShopDrawingError(false);
    setShopDrawingMessage("");
    try {
      const label = `${drawing.number} · ${drawing.title}`;
      const response = await fetch("/api/job-documents", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ portalJobId: job.portalJobId, documentLink: secureLink, documentLinkLabel: label }),
      });
      const result = await response.json().catch(() => ({})) as { error?: string; documentLink?: string; documentLinkLabel?: string };
      if (!response.ok) throw new Error(result.error || "The approved drawing could not be shared.");
      setState((current) => ({
        ...current,
        jobs: current.jobs.map((item) => item.id !== job.id ? item : {
          ...item,
          documentLink: result.documentLink ?? secureLink,
          documentLinkLabel: result.documentLinkLabel ?? label,
          shopDrawings: (item.shopDrawings ?? []).map((candidate) => ({ ...candidate, sharedWithEmployees: candidate.id === drawing.id })),
        }),
      }));
      setShopDrawingMessage(`${drawing.number} is now the drawing link on the employee job list. Sharing another item will replace it there.`);
    } catch (error) {
      setShopDrawingError(true);
      setShopDrawingMessage(error instanceof Error ? error.message : "The approved drawing could not be shared.");
    } finally {
      setShopDrawingAction(null);
    }
  };

  const unshareShopDrawing = async (drawing: ShopDrawing) => {
    if (!job?.portalJobId || !shopDrawingIsSharedWithEmployees(job, drawing) || shopDrawingAction) return;
    setShopDrawingAction("unsharing");
    setShopDrawingError(false);
    setShopDrawingMessage("");
    try {
      await clearEmployeeJobListLink();
      setState((current) => ({
        ...current,
        jobs: current.jobs.map((item) => item.id !== job.id ? item : {
          ...item,
          documentLink: "",
          documentLinkLabel: "",
          shopDrawings: (item.shopDrawings ?? []).map((candidate) => ({ ...candidate, sharedWithEmployees: false })),
        }),
      }));
      setShopDrawingMessage(`${drawing.number} was removed from the employee job list and remains in the register.`);
    } catch (error) {
      setShopDrawingError(true);
      setShopDrawingMessage(error instanceof Error ? error.message : "The drawing could not be removed from the employee job list.");
    } finally {
      setShopDrawingAction(null);
    }
  };
  ...
  const deleteShopDrawing = (drawing: ShopDrawing) => {
    ...
    setState((current) => ({
      ...current,
      jobs: current.jobs.map((item) => item.id === job.id ? { ...item, shopDrawings: (item.shopDrawings ?? []).filter((candidate) => candidate.id !== drawing.id) } : item),
    }));
    setShopDrawingEditor(null);
    setShopDrawingMessage(`${drawing.number} deleted. Drawings with submission history are closed instead of deleted.`);
    setShopDrawingError(false);
    setShopDrawingAction(null);
  };
    ...
    try {
      ...
      setState((current) => ({
        ...
        jobs: current.jobs.map((item) => item.id !== job.id ? item : {
          ...
          portalSiteName: saved.siteName ?? null,
  ...
  if (job) {
    ...
    const shopDrawings = job.shopDrawings ?? [];
    const shopDrawingMetrics = {
      open: shopDrawings.filter((drawing) => isOpenShopDrawing(drawing.status)).length,
      awaitingVendor: shopDrawings.filter((drawing) => drawing.responsibility === "Vendor" && isOpenShopDrawing(drawing.status)).length,
      withConsultant: shopDrawings.filter((drawing) => drawing.responsibility === "Consultant / Client" && isOpenShopDrawing(drawing.status)).length,
      revisionsRequired: shopDrawings.filter((drawing) => drawing.status === "Revise and resubmit").length,
      approved: shopDrawings.filter((drawing) => isApprovedShopDrawing(drawing.status)).length,
      overdue: shopDrawings.filter((drawing) => shopDrawingDateState(drawing) === "overdue").length,
    };
    const normalizedDrawingSearch = shopDrawingSearch.trim().toLocaleLowerCase("en-CA");
    const visibleShopDrawings = shopDrawings.filter((drawing) => {
      if (shopDrawingFilter === "Open" && !isOpenShopDrawing(drawing.status)) return false;
      if (shopDrawingFilter === "Approved" && !isApprovedShopDrawing(drawing.status)) return false;
      const haystack = `${drawing.number} ${drawing.title} ${drawing.division} ${drawing.vendorName} ${drawing.consultant} ${drawing.status} ${drawing.responsibility}`.toLocaleLowerCase("en-CA");
      return haystack.includes(normalizedDrawingSearch);
    });
    const editorDrawing = shopDrawingEditor?.drawingId ? shopDrawings.find((drawing) => drawing.id === shopDrawingEditor.drawingId) ?? null : null;
    const editorHasChanges = shopDrawingEditor ? JSON.stringify(shopDrawingEditor.draft) !== shopDrawingEditor.original : false;
      ...
      <div className="page-stack job-detail-page">
        ...
        <section className="panel job-summary-panel">
          ...
          {!jobInfoEditing ? <div className="job-summary-facts">
            ...
            <div><span>Site</span><strong>{(job.portalSiteName ?? acceptedBasis.quote?.site ?? job.portalAddress) || "Not entered"}</strong><small>{job.portalSiteName == null && hasAcceptedQuote ? "Accepted quote" : "Official Portal job"}</small></div>
        ...
        <section className={`panel job-shop-drawing-summary ${shopDrawingMetrics.overdue ? "has-overdue" : ""}`}>
          <div className="panel-heading">
            <div><span className="eyebrow">SHOP DRAWING STATUS</span><h2>{shopDrawings.length ? `${shopDrawingMetrics.open} open submission${shopDrawingMetrics.open === 1 ? "" : "s"}` : "No shop drawings entered"}</h2><p>OneDrive files stay linked to the controlled register. Approved current revisions can be shared to the employee job list.</p></div>
            <button className="button secondary compact" type="button" onClick={() => setTab("shop-drawings")}>{shopDrawings.length ? "Open register" : "＋ Add first drawing"}</button>
          </div>
          ...
          {hasAcceptedQuote && <div className="job-insight-column">
            ...
            <aside className="panel forecast-panel">
              ...
              <div className="forecast-lines">
                ...
                <label><span>Estimated cost still to come</span><div className="input-prefix"><span>$</span><ClearableNumberInput value={job.estimateToComplete} onValueChange={(value) => setState((current) => ({ ...current, jobs: current.jobs.map((item) => item.id === job.id ? { ...item, estimateToComplete: value ?? 0 } : item) }))} /></div></label>
        ...
        {tab === "shop-drawings" && <section className="panel shop-drawing-panel">
          ...
          {shopDrawingEditor && <section className="shop-drawing-editor" aria-label={shopDrawingEditor.mode === "create" ? "New shop drawing" : `Edit ${shopDrawingEditor.draft.number}`}>
            <div className="shop-drawing-editor-heading">
              <div><span className="eyebrow">{shopDrawingEditor.mode === "revision" ? "NEW REVISION" : shopDrawingEditor.mode === "create" ? "NEW REGISTER ITEM" : "CURRENT REVISION"}</span><h3>{shopDrawingEditor.mode === "revision" ? `${shopDrawingEditor.draft.number} · Revision ${(editorDrawing?.revision ?? 0) + 1}` : shopDrawingEditor.mode === "create" ? "Add a required submission" : `${shopDrawingEditor.draft.number} · Revision ${editorDrawing?.revision ?? 0}`}</h3><p>Opening this record does not create a revision. A revision is added only when a new revision is saved.</p></div>
              <button className="icon-close" type="button" aria-label="Close shop drawing editor" onClick={() => setShopDrawingEditor(null)}>×</button>
            </div>
            <datalist id={`shop-drawing-vendors-${job.id}`}>{state.vendors.filter((vendor) => vendor.status === "Active").map((vendor) => <option key={vendor.id} value={vendor.name} />)}</datalist>
            <div className="form-grid three-column shop-drawing-form">
              <label className="field"><span>SD number *</span><input aria-label="SD number" value={shopDrawingEditor.draft.number} onChange={(event) => updateShopDrawingDraft("number", event.target.value)} maxLength={30} /></label>
              ...
              <label className="field full"><span>Current revision OneDrive link <small>Optional</small></span><input aria-label="Current revision OneDrive link" type="url" inputMode="url" value={shopDrawingEditor.draft.oneDriveUrl} onChange={(event) => updateShopDrawingDraft("oneDriveUrl", event.target.value)} placeholder="https://...sharepoint.com/..." /></label>
