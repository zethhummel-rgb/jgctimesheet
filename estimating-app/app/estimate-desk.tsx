        ...
        {style === "jgc-classic" ? (
          <>
            ...
            <section className={`hybrid-meta ${projectAddress ? "has-address" : ""}`}>
              <div className="document-client"><span>Prepared for</span><strong>{client?.name || "Client not selected"}</strong>{projectAddress && <p className="document-address">{projectAddress}</p>}{(quote.proposalAttention || client?.contact) && <p>Attention: {quote.proposalAttention || client?.contact}</p>}</div>
              <div className="document-project"><span>Project</span><strong>{quote.project || "Project not named"}</strong>{quote.site && <p>{quote.site}</p>}{changeNotice && <p>Change: {quote.changeTitle || "Change not named"}</p>}{quote.reference && <p>Reference: {quote.reference}</p>}</div>
