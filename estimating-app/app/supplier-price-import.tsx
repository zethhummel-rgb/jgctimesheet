...
export function SupplierCatalogSection({ search, divisions, refreshKey, onImport }: { search: string; divisions: string[]; refreshKey: number; onImport: () => void }) {
  ...
  const saveItem = async () => {
    if (!draft) return;
    const response = await fetch("/api/supplier-catalog", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: draft.id, productName: draft.productName, division: draft.division, unit: draft.unit, netCost: draft.netCost, active: draft.active }) });
  ...
  return <section className="supplier-catalog-block">
    <div className="supplier-catalog-heading"><div><span className="eyebrow">MATERIAL PRICE BOOK</span><h2>Supplier material pricing</h2><p>Material-only pricing from supplier PDFs and completed JGC Excel sheets. Updates match the material name or supplier reference—not the spreadsheet row.</p></div><button className="button primary" onClick={onImport}>⇧ Import or update prices</button></div>
