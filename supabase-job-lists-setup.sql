...
begin
  ...
  for v_list in
    ...
    where l.status = 'open'
      ...
      and l.reminder_at is not null
      and l.reminder_sent_at is null
      and l.reminder_at <= now()
    order by l.reminder_at
  ...
  loop
    ...
    loop
      insert into public.notifications (
        ...
        'job_list_reminder',
        'Job note reminder: ' || v_list.title,
        v_list.job_number || ' - ' || v_list.job_name || ' | '
          || v_remaining::text || case when v_remaining = 1 then ' item remaining' else ' items remaining' end,
        'job-lists.html?list=' || v_list.id::text,
        v_recipient.id,
        ...
        'job_lists',
        v_list.id::text,
        'job-list-reminder:' || v_list.id::text || ':'
          || extract(epoch from v_list.reminder_at)::bigint::text || ':' || v_recipient.id::text,
        jsonb_build_object(
          'list_id', v_list.id,
          'job_number', v_list.job_number,
          'job_name', v_list.job_name,
          'remaining_items', v_remaining,
          'reminder_at', v_list.reminder_at
        ),
        v_list.created_by,
        v_list.created_by_name,
        now() + interval '7 days',
        now()
      )
