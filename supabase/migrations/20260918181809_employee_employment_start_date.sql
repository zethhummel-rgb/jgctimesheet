-- Reuse the existing hire date; preserve unknown dates on existing accounts.
alter table public.profiles alter column hire_date set default ((now() at time zone 'America/Toronto')::date);
comment on column public.profiles.hire_date is 'Employment Start Date. Accounting and timesheets expect weekdays on or after this date; NULL preserves legacy expectations.';

CREATE OR REPLACE FUNCTION public.accounting_autofill_leave_timesheet(p_profile_id uuid, p_week_start date, p_days text[] DEFAULT '{}'::text[], p_entry_type text DEFAULT 'vacation'::text, p_leave_type text DEFAULT 'paid'::text, p_note text DEFAULT ''::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
begin
  ...
  select profile.display_name, profile.worker_key, profile.hire_date
  into v_worker_name, v_worker_key, v_employment_start
  from public.profiles profile
  ...
  if v_employment_start is not null and p_week_start + 6 < v_employment_start then
    raise exception 'No timesheet is required before the Employment Start Date.';
  end if;
  ...
  if exists (
    select 1 from unnest(v_days) day_name
    where p_week_start + array_position(array['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'], day_name) - 1 < v_employment_start
  ) then
    raise exception 'Days before the Employment Start Date cannot be auto-filled.';
  end if;

  if v_employment_start > p_week_start + 5 and not exists (
    select 1 from public.timesheet_entries entry
    where entry.profile_id = p_profile_id and entry.week_start = p_week_start
      and p_week_start + array_position(array['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'], entry.day_of_week) - 1 >= v_employment_start
  ) then
    raise exception 'This week has no required weekdays or entered time after the Employment Start Date.';
  end if;
  ...
  where (v_employment_start is null or p_week_start + required.day_offset >= v_employment_start)
  and not exists (
    select 1
    from public.timesheet_entries entry
    where entry.profile_id = p_profile_id
