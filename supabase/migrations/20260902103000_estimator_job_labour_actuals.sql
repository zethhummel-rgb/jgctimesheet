...
as $$
begin
  if not (select private.jgc_has_accounting_access()) then
    raise insufficient_privilege using
      message = 'Accounting access is required to view employee labour costs.';
  end if;
...
comment on function public.get_estimator_job_labour_actuals() is
  'Returns admin-only job labour summaries for the Estimator. Loaded cost matches the Accounting workbook: regular wages plus night premium, with 40 percent burden. Raw pay rates are never returned.';
